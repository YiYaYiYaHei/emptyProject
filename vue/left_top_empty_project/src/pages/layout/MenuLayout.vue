...
<template>
  <div class="application--left-top-menu">
    ...
    <el-menu :default-active="activeIndex" class="el-menu-top" mode="horizontal" background-color="#545c64" text-color="#fff" active-text-color="#ffd04b">
      <template v-for="(item, i) in menuList">
        <template v-if="!item.hidden">
          <!-- 一级菜单 -->
          <el-menu-item :index="item.label" :key="`menu_${i}`" @click="topMenuClick(item, 'click')">
    ...
    <!-- 左侧菜单 -->
    <el-menu :default-active="leftActiveIndex" class="el-menu-left" background-color="#545c64" text-color="#fff" active-text-color="#ffd04b" :style="{'width': !!leftMenuList.length ? '210px': 0}" router>
      <template v-for="(item, i) in leftMenuList">
        <!-- 二级菜单 -->
        <template v-if="item.children&&item.children.length&&!item.hidden">
          <el-submenu :key="`menu_left_${i}`" :index="item.label">
            <template slot="title">
              <i class="menu-icon" v-if="!!item.icon" :class="item.icon"></i>
              <span>{{item.label}}</span>
            </template>
            <!-- 三级菜单 -->
            <el-menu-item v-for="(son, k) in item.children" :key="`menu_left_${i}_${k}`" v-if="!son.hidden" :index="son.label" :route="son.path">
        ...
        <template v-else-if="!item.hidden">
          <!-- 二级菜单 -->
          <el-menu-item :key="`menu_left_${i}`" :index="item.label" :route="item.path">
            <i class="menu-icon" v-if="!!item.icon" :class="item.icon"></i>
            <span>{{item.label}}</span>
          </el-menu-item>
        </template>
      ...
      if (!!this.leftMenuList.length) {
        ...
        let obj = !!_parent
          ? !!(_parent.children || []).length
            ...
            : _parent
          : { label: '', path: '' }
        /* path = !!(this.leftMenuList[0].children || []).length ? this.leftMenuList[0].children[0].path : this.leftMenuList[0].path; 
                name = !!(this.leftMenuList[0].children || []).length ? this.leftMenuList[0].children[0].label : this.leftMenuList[0].label;  */
        path = obj.path
        name = obj.label
      }
